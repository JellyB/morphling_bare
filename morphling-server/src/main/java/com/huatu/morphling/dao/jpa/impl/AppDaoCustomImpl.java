...
public class AppDaoCustomImpl extends BaseJpaDao implements AppDaoCustom {
    ...
    public AppInstanceVO findByInstanceId(int id) {
        String sql = "select a.port as port,a.context_path as contextPath,ac.id as id,c.host_address as host,c.name as clientName,a.name as appName,ac.status as status,ac.update_time as updateTime  from client c,app a,app_instance ac where ac.app_id=a.id and ac.client_id=c.id and ac.id=? and c.state=1 and ac.state=1";
    ...
    public List<AppInstanceVO> findInstances(List<Integer> instanceIds) {
        ...
        String sql = "select a.port as port,c.port as clientPort,a.context_path as contextPath,ac.id as id,c.host_address as host,c.name as clientName,a.name as appName,ac.status as status,ac.update_time as updateTime  from client c,app a,app_instance ac where ac.app_id=a.id and ac.client_id=c.id and ac.id in ("+id+") and c.state=1 and ac.state=1";
        return findToTransformerBySql(sql,AppInstanceVO.class);
    }

    @Override
    public List<AppInstanceVO> findInstances(int appId) {
        String sql = "select a.port as port,a.context_path as contextPath,ac.current_version as currentVersion,ac.id as id,c.host_address as host,c.name as clientName,a.name as appName,ac.status as status,ac.update_time as updateTime  from client c,app a,app_instance ac where ac.app_id=a.id and ac.client_id=c.id and ac.app_id=? and c.state=1 and ac.state=1";
