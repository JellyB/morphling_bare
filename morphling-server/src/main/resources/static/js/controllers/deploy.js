        ...
        headers: [
            ...
            {name:"健康检测",class:"text-center",compile:true,field:"_health",formatter:function(value,row){
                ...
                return "<span ng-if='!monitor.healthStatus["+row.id+"]'><i class='fa fa-spin fa-spinner '></i></span>" +
                    "<span ng-if='monitor.healthStatus["+row.id+"]' class='label cusor-pointer' ng-class=\"{'UP':'bg-success','DOWN':'bg-danger','ERROR':'bg-dark'}[monitor.healthStatus["+row.id+"]]\"   endpoint-show =\"{{monitor.healthDetail["+row.id+"]}}\"  >{{{'UP':'正常','DOWN':'异常','ERROR':'连接失败'}[monitor.healthStatus["+row.id+"]]}}</span>";
            }},
